...
export default function SkillDetailModal({ skillName, onClose }: Props) {
  ...
  return (
    ...
    >
      <div
        className="bg-surface rounded-xl shadow-2xl border border-border-light w-full max-w-2xl max-h-[80vh] flex flex-col m-4"
        onClick={e => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center justify-between px-4 py-3 border-b border-border-light shrink-0">
          <h2 className="text-sm font-semibold text-text-primary">{skillName}</h2>
        ...
        {/* Content */}
        <div className="flex-1 overflow-y-auto px-4 py-3 text-xs text-text-secondary prose prose-sm max-w-none">
          {loading ? (
            ...
            <ReactMarkdown remarkPlugins={[remarkGfm]} components={{
              ...
              a: ({ href, children }) => (
                <a href={href} target="_blank" rel="noopener noreferrer" className="text-brand hover:text-brand-dark hover:underline">{children}</a>
              ),
            }}>
              {content}
            </ReactMarkdown>
          ) : (
            <p className="text-text-muted text-center py-8">No content available for this skill.</p>
          )}
        </div>
      </div>
    </div>
