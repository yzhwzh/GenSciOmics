...
export default function FisherTable({
  ...
  if (!data?.fisher) return <div className="text-xs text-text-muted py-4 text-center">No data</div>
  ...
  return (
    <div className="overflow-auto max-h-full">
      ...
      {fisher.cell_types.length > maxCt && (
        <div className="text-[9px] text-text-muted text-center py-0.5">Showing {maxCt} of {fisher.cell_types.length} cell types</div>
      )}
    </div>
  )
}
